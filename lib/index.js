...
const generateCriticalCssWrapped = async function generateCriticalCssWrapped(options, { forceTryRestartBrowser } = {}) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (!forceTryRestartBrowser && !runningBrowswer) {
        debuglog('Browser unexpecedly not opened - crashed? ' + '\nurl: ' + options.url || (options.htmlString && 'using htmlString with length of ' + options.htmlString.length) + '\ncss length: ' + options.cssString.length);
        try {
    ...
    if (formattedCss.trim().length === 0) {
      ...
      debuglog('Note: Generated critical css was empty for URL: ' + options.url || options.htmlString && 'using htmlString with length of ' + options.htmlString.length);
      resolve('');
      return;
    }
