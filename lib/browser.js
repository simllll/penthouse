...
async function launchBrowserIfNeeded({ getBrowser, width, height }) {
  if (browser) {
    return;
  }
...
async function closeBrowser({ forceClose, unstableKeepBrowserAlive }) {
...
async function restartBrowser({ getBrowser, width, height }) {
  let browserPages;
  if (browser) {
