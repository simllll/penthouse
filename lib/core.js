...
async function preparePage({
  ...
  if (currentViewport.width !== width || currentViewport.height !== height) {
    setViewportPromise = page.setViewport({ width, height }).then(() => debuglog('viewport size updated'));
  }
  ...
  if (cookies) {
    try {
      setCookiesPromise = page.setCookie(...cookies).then(() => debuglog('cookie(s) set: ' + JSON.stringify(cookies)));
    } catch (e) {
...
async function pruneNonCriticalCssLauncher({
  ...
  return new Promise(async (resolve, reject) => {
    ...
    async function cleanupAndExit({ error, returnValue }) {
      ...
      if (page) {
        ...
        if (cookies && cookies.length) {
          try {
            resetPromises.push(page.deleteCookie(...cookies).then(() => debuglog('cookie(s) reset: ')));
          } catch (e) {
    ...
    const loadPagePromise = loadPage(page, url, htmlString, timeout, pageLoadSkipTimeout);

    // turn css to formatted selectorlist [NOT BLOCKING]
    debuglog('turn css to formatted selectorlist START');
    const buildSelectorProfilePromise = (0, _selectorsProfile2.default)(ast, forceInclude).then(res => {
    ...
    try {
      criticalSelectors = await page.evaluate(_pruneNonCriticalSelectors2.default, {
        selectors,
        renderWaitTime,
        maxElementsToCheckPerSelector
    ...
    if (takeScreenshots) {
      ...
      await page.evaluate(_replacePageCss2.default, { css }).then(() => {
